const scraperObject = {
    ...
    async executeScrapper(browser) {
        const unicoPromise = new Promise(async (resolve, reject) => {
            ...
            await page.waitForSelector('#currency-design').catch((err) => reject(err))
            let currencies = await page.$$eval('div#currency-design', values => {
                let currenciesArray = []
                values = values.map((el) => el.querySelector('table > tbody')).map((el) => el.innerText)
                console.log(values[0])
                let currencyStrings = values[0].replaceAll("\t", "|").replaceAll('\n', "|").replaceAll('|', ' | ').replaceAll('Moeda |  |  | Compra |  |  | Venda |  | ', '')
