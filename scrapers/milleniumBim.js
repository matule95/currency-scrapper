const scraperObject = {
    ...
    async executeScrapper(browser){
        const milleniumPromise = new Promise(async (resolve, reject) => {
            ...
            let currencies = await page.$$eval('.rates-card-one > .rates-values > .values', values => {
                values = values.map((el) => {
                    const currency = {}
                    currency.currency = el.querySelector('span').innerText
                    currency.buy = parseFloat(el.querySelector('span.float-right.buy').innerText.replace(/,/, '.'))
