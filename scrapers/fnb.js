    ...
    const fnbPromise = new Promise(async (resolve, reject) => {
      ...
      await page.waitForSelector("div#cambiosGrid").then(async (data) => {
        let currencies = await page.$$eval(
          "div#cambiosGrid > div > table > tbody",
          (values) => {
            let htmlElements = [];
            ...
            const strings = htmlElements
              .toString()
              .trim()
              .split(/<td align="center">(.*?)<\/td>/);
            const sanitizedArray = [];
            strings.forEach((string, index) => {
              const regex = RegExp("/\\s/g");
              const sanitizedString = string.toString();
              if (
                sanitizedString.length < 8 &&
                sanitizedString.length > 2 &&
                !sanitizedString.includes("<") &&
              ...
              if (
                ...
                if (pattern.test(string)) {
                  ...
                  currency.buy = parseFloat(
                    sanitizedArray[index + 1].replace(/,/, ".")
                  );
                  currency.sell = parseFloat(
                    sanitizedArray[index + 2].replace(/,/, ".")
                  );
            ...
            return {
              ...
              currencies: sanitizedCurrencies.sort(
